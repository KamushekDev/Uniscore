...
class RoundedWithCutoutCircleBorder extends OutlinedBorder {
  /// Creates a rounded rectangle border.
  ///
  /// The arguments must not be null.
  const RoundedWithCutoutCircleBorder({
  ...
  Path getInnerPath(Rect rect, {TextDirection? textDirection}) {
    var path = Path()..addRRect(borderRadius.resolve(textDirection).toRRect(rect).deflate(side.width));

    var size = path.getBounds();
    var circle = Path()..addOval(Rect.fromCircle(center: Offset(size.width / 2, size.height), radius: cutoutRadius).deflate(side.width));
  ...
  Path getOuterPath(Rect rect, {TextDirection? textDirection}) {
    var path = Path()..addRRect(borderRadius.resolve(textDirection).toRRect(rect));

    var size = path.getBounds();
    var circle = Path()..addOval(Rect.fromCircle(center: Offset(size.width / 2, size.height), radius: cutoutRadius));
...
class _RoundedRectangleToCircleBorder extends OutlinedBorder {
  ...
  @override
  Path getInnerPath(Rect rect, {TextDirection? textDirection}) {
    return Path()..addRRect(_adjustBorderRadius(rect, textDirection)!.toRRect(_adjustRect(rect)).deflate(side.width));
  }
